...
public class TestAnnotationProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        Set<? extends Element> getPermissionsElement = roundEnvironment.getElementsAnnotatedWith(TestAnnotation.class);
        for (Element element : getPermissionsElement) {
            ...
            String[] permissions = executableElement.getAnnotation(TestAnnotation.class).value();
            String methodName = executableElement.getSimpleName().toString();
            String returnType = executableElement.getReturnType().toString();

            note(String.format("%s %s", returnType, methodName));
            // create the real file to be compiled
            createFile(packageName + "Test", enclosingName, permissions, methodName, returnType);
        }
        return false;
    }
